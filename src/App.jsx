...
const App = () => {
  ...
  return (
    <div className="relative w-full h-[100dvh] bg-slate-900 overflow-hidden font-sans select-none text-slate-100">
          ...
          <div className="flex flex-col gap-2 items-end">
             <div className="flex items-center gap-3 bg-slate-900/60 backdrop-blur-md border border-white/5 rounded-full px-4 py-1.5 shadow-xl">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-bold text-pink-100">128</span>
                </div>
             </div>
             ...
             <div className="flex items-center gap-2 bg-slate-900/40 backdrop-blur-sm rounded-full px-3 py-1 border border-white/5">
                ...
                <span className="text-[10px] font-mono text-blue-200">{energy}/100</span>
             </div>
          </div>
        </div>
      </header>
      ...
      {/* The Panel Itself */}
      <div 
        className={`absolute bottom-0 left-0 right-0 z-50 bg-slate-900/95 backdrop-blur-2xl border-t border-white/10 rounded-t-[3rem] shadow-[0_-10px_40px_rgba(0,0,0,0.5)] transition-transform duration-500 cubic-bezier(0.32, 0.72, 0, 1) ${activePanel ? 'translate-y-0' : 'translate-y-[110%]'}`}
      ...
      <nav className="absolute bottom-6 left-4 right-4 z-50 pb-safe-bottom">
        <div className="relative">
          ...
          <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-xl border border-white/5 rounded-2xl shadow-2xl"></div>
          ...
          <div className="relative flex justify-between items-center px-4 py-2">
            ...
            {/* Main Action Button (Floating above dock) - No Icon/Dot */}
            <div className="relative -top-8 mx-4">
               <button 
                 className="group relative w-20 h-20 rounded-full bg-gradient-to-b from-pink-500 to-blue-700 shadow-[0_0_30px_rgba(236,72,153,0.4)] flex items-center justify-center transform transition-all duration-300 hover:scale-105 active:scale-95 border-4 border-slate-900"
...
const DockItem = ({ icon: Icon, label, isActive, onClick }) => (
  ...
  >
    <Icon className={`w-6 h-6 mb-1 transition-colors ${isActive ? 'text-pink-400' : 'text-slate-400'}`} />
    <span className={`text-[10px] uppercase font-bold tracking-wider ${isActive ? 'text-pink-200' : 'text-slate-500'}`}>
      {label}
    </span>
    {isActive && (
      <div className="absolute bottom-1 w-1 h-1 bg-pink-400 rounded-full shadow-[0_0_8px_cyan]"></div>
    )}
  </button>
);
...
const InventorySlot = ({ index }) => {
  ...
  return (
    <div className={`aspect-square rounded-2xl border ${isRare ? 'border-pink-500/30 bg-pink-500/10' : 'border-white/5 bg-white/5'} flex items-center justify-center relative overflow-hidden group active:scale-95 transition-transform`}>
      {!isEmpty && (
        <>
          <div className="absolute inset-0 bg-gradient-to-tr from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
          <div className={`w-8 h-8 rounded-full ${isRare ? 'bg-pink-400/20' : 'bg-slate-400/20'} animate-pulse`}></div>
